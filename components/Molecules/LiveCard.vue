<template>  
  <li class="live-card">
    <div class="date">
      <p class="ymd">{{ $dayjs(detail.date).format('YYYY/M/D (ddd)') }}</p>
    </div>
    <div class="detail">
      ...
      <p class="iyu-time" v-if="detail.timeIyu">※iyuの出演は{{ detail.timeIyu }}頃の予定です。</p>
      <p class="ticket">{{ detail.ticket }}</p>
      <p class="with">w) {{ detail.with ? detail.with.join(' / ') : "未定"}}</p>
    </div>
  </li>
</template>
